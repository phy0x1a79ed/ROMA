...
class CacheStatistics(BaseModel):
    ...
    time_saved_ms: int = Field(
        default=0, description="Estimated time saved by cache (milliseconds)"
    )
...
class TaskSnapshot(BaseModel):
    ...
    subgraph_id: Optional[str] = Field(
        default=None, description="Associated subgraph ID"
    )
    dependencies: List[str] = Field(
        default_factory=list, description="Task dependency IDs"
    )
...
class DAGSnapshot(BaseModel):
    ...
    completed_tasks: Set[str] = Field(
        default_factory=set, description="Completed task IDs"
    )
    ...
    subgraphs: Dict[str, "DAGSnapshot"] = Field(
        default_factory=dict, description="Nested subgraph snapshots"
    )
...
class CheckpointData(BaseModel):
    ...
    trigger: CheckpointTrigger = Field(
        description="Event that triggered checkpoint creation"
    )
    state: CheckpointState = Field(
        default=CheckpointState.CREATED, description="Checkpoint validity state"
    )
    ...
    recovery_strategy: RecoveryStrategy = Field(
        default=RecoveryStrategy.PARTIAL, description="Preferred recovery strategy"
    )
    failed_task_ids: Set[str] = Field(
        default_factory=set, description="Tasks that failed and need retry"
    )
    preserved_results: Dict[str, Any] = Field(
        default_factory=dict, description="Results to preserve during recovery"
    )

    # Runtime context
    solver_config: Dict[str, Any] = Field(
        default_factory=dict, description="RecursiveSolver configuration"
    )
    module_states: Dict[str, Dict[str, Any]] = Field(
        default_factory=dict, description="Module instance states"
    )

    # Cache statistics
    cache_stats: Optional[CacheStatistics] = Field(
        default=None, description="Cache performance stats at checkpoint time"
    )
...
class RecoveryPlan(BaseModel):
    ...
    tasks_to_preserve: List[str] = Field(
        description="Task IDs with results to preserve"
    )

    # State restoration
    restore_dag_state: bool = Field(
        default=True, description="Whether to restore full DAG state"
    )
    restore_module_states: bool = Field(
        default=False, description="Whether to restore module states"
    )

    # Recovery parameters
    reset_retry_counts: bool = Field(
        default=False, description="Whether to reset task retry counters"
    )
    apply_backoff: bool = Field(
        default=True, description="Whether to apply exponential backoff"
    )
...
class CheckpointConfig(BaseModel):
    ...
    storage_path: Path = Field(
        default=Path(".checkpoints"), description="Checkpoint storage directory"
    )

    # Checkpoint creation
    auto_checkpoint_triggers: List[CheckpointTrigger] = Field(
        default=[
            CheckpointTrigger.BEFORE_PLANNING,
            CheckpointTrigger.BEFORE_AGGREGATION,
        ],
        description="Events that automatically create checkpoints",
    )
    max_checkpoints: int = Field(
        default=10, description="Maximum checkpoints to retain"
    )

    # Checkpoint retention
    max_age_hours: float = Field(
        default=24.0, description="Maximum checkpoint age in hours"
    )
    cleanup_interval_minutes: int = Field(
        default=60, description="Cleanup interval in minutes"
    )

    # Recovery settings
    default_recovery_strategy: RecoveryStrategy = Field(
        default=RecoveryStrategy.PARTIAL, description="Default recovery strategy"
    )
    preserve_partial_results: bool = Field(
        default=True, description="Preserve partial results during recovery"
    )

    # Compression and storage
    compress_checkpoints: bool = Field(
        default=True, description="Compress checkpoint data"
    )
    verify_integrity: bool = Field(
        default=True, description="Verify checkpoint integrity on load"
    )
    ...
    periodic_interval_seconds: float = Field(
        default=30.0, description="Interval between periodic checkpoints (seconds)"
    )
